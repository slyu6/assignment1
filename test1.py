...
# label split
random.shuffle(test_lab)
n = len(test_lab)//2
# separate
test_lab1 = np.array(test_lab[0:n])
train_lab = np.array(test_lab[n:])
...
n = len(test_sen)//2
# separate
test_set = np.array(test_sen[0:n])
train_set = np.array(test_sen[n:])


dist_sum = []
# test
print(np.array(train_set[0][0]))
...
print("x", train_set[-1,1])
print("y", test_set[-1,1])
...
def KNN_classify(k,X_train,x_train,Y_test):
    ...
    for i in range(num_test):
        print("x", X_train[-1,1])
        print("y", Y_test[-1,1])
        distances = np.sum(np.abs(X_train[-1,1] - Y_test[-1,1]) + np.abs(X_train[-1,0] - Y_test[-1,0]))
...
x = KNN_classify(3, train_set, train_lab, test_set,)
